...
const uri = 'mongodb+srv://admin:28295609@cluster0.6kcmggh.mongodb.net/?appName=Cluster0';
...
async function main() {
    ...
    console.log('\n--- Users with strikes ---');
    strikers.sort((a, b) => (b[1].strikes || 0) - (a[1].strikes || 0));
    for (const [uid, data] of strikers) {
        console.log(`  ${uid}: strikes=${data.strikes}, banned=${data.banned}`);
    }
    await client.close();
}
