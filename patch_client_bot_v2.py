...
new_balance_logic = r"""if state == "awaiting_balance_amount":
        try:
            import string
            import random
            ...
            return"""
code = re.sub(old_balance_logic, new_balance_logic.replace('\\n', '\\\\n'), code, count=1, flags=re.DOTALL)

# 7. Add `pay_` handler into `button_handler`
pay_handler = r"""    elif data.startswith("pay_"):
        parts = data.split("_")
        method = parts[1]
        order_id = parts[2]
...
"""
# .replace() does not process regex escapes, so `\\n` is safe.
...
new_shop_kb = r"""def get_shop_keyboard(user_id=None):
    keyboard = [
...
new_prod_logic = r"""elif data.startswith("product_"):
...
code = re.sub(prod_logic, new_prod_logic.replace('\\n', '\\\\n'), code, flags=re.DOTALL)

# Remove extra `)` bug from earlier replace
code = code.replace('parse_mode="HTML")\n        )', 'parse_mode="HTML")')
...
new_price_map = r"""duration_map = {"1d": "1 Days", "3d": "3 Days", "7d": "7 Days", "15d": "15 Days", "30d": "30 Days"}
        price_map = {"1d": "2.00", "3d": "4.00", "7d": "6.00", "15d": "10.00", "30d": "18.00"}"""
code = re.sub(price_map_logic, new_price_map, code, flags=re.DOTALL)

# 11. Remove web_products check in buy_
web_prod_loop = r"""        web_products = load_web_products\(\).*?break"""
code = re.sub(web_prod_loop, "", code, flags=re.DOTALL)
