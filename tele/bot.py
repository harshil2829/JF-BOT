...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "shop":
        ...
        try:
            with open("/Users/cashify/Desktop/JF BOT/trial_history.json", "r") as f:
                hist = json.load(f)
            if not hist:
                raise Exception
        except:
            kb = [[InlineKeyboardButton("« Back", callback_data="admin_panel_cb")]]
            await query.edit_message_text("❌ No trial logs found yet.", reply_markup=InlineKeyboardMarkup(kb))
            return
